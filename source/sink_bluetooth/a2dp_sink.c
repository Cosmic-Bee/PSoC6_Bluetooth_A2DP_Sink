...
void a2dp_sink_data_cback( uint8_t* p_rx_media, uint32_t media_len )
{
    ...
    for (i = 0; i < num_frames; i++)
    {
        ...
        status = i2s_audio_data_write((void *)pOut, audio_frames_to_write,
                                           &audio_frames_written);
        if (status != WICED_BT_SUCCESS)
        {
           WICED_BT_TRACE ("i2s_audio_data_write () failed. Status: %d", status);
        }
